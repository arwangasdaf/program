...
object launch {
  def main(args: Array[String]): Unit = {
    ...
    val SAALLRDD = testRdd.filter(item => new String(item._2).equals("ITEM")).repartition(440).cache()
    // item._1  S   item._4  A
    ...
    SAALLRDD.foreachPartition{
      case triple => {
        ...
        val myTable = pool2.getTable("SATDN")
        ...
        triple.foreach{
          case item =>{
                              ...
                              if(qual_2.equals("ITEM")){
                                ...
                                for(ddtype <- result_5.raw()){
                                  if(Bytes.toString(ddtype.getQualifier).equals("NODE_SERVICE_TYPE")){
                                    ...
                                    myTable.setAutoFlush(false, false)
                                    myTable.setWriteBufferSize(5*1024*1024)
                                    val p = new Put(Bytes.toBytes(item._4 +"_"+ item._1))
            ...
            for ((k: String, v: Int) <- map) {
              // K : D.type   v : 1
              //将计算所得结果 (S , A , T , d.type , n) 写入到数据库中
              //Hbase数据表的名称为 ASDN , rowkey为 A_S_T 列族为:INFO 列簇为 D.type value 为 n
              //首先根据 A 查找 ATN 这张表
              // item._1  S   item._4  A
              var rowkey_6 = item._4   // A
              var get7 = new Get(rowkey_6.getBytes())
              var result_6 = table_ATN.get(get7)
              for(cd <- result_6.raw()) {
                var time = Bytes.toString(cd.getQualifier)  //得到时间 T
                var click = Bytes.toString(cd.getValue)       //得到发生次数
                var n = v * click.toInt                     //时间 T 发生的次数 * d.type 的个数
                /*
                * 写入到 Hbase 表里面去
                *  // (S , A , T , d.type , n)
                * */
                val p = new Put(Bytes.toBytes(item._4 +"_"+ item._1 +"_"+time))
                p.add(Bytes.toBytes("INFO"),Bytes.toBytes(k),Bytes.toBytes(n.toString))
                myTable.put(p)
              }
            }
