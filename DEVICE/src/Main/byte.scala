...
object byte {
  def cal(SARDD:RDD[(String,String,String,String,String)], sc:SparkContext , config:Configuration)={
    ...
    val INRDD = SARDD.filter(item => (new String(item._3).equals("STORE_DATA")))
      ...
      triple => {
        ...
        triple.foreach{
          item=>{
            ...
            p.add(Bytes.toBytes("ITEM"), Bytes.toBytes("BYTE_avg"),Bytes.toBytes(item._2.toString.substring(0,4)))       //写入平均值
            p.add(Bytes.toBytes("ITEM"), Bytes.toBytes("BYTE_max_1"),Bytes.toBytes((item._3.toString.substring(0,4))))
            p.add(Bytes.toBytes("ITEM"), Bytes.toBytes("BYTE_max_2"),Bytes.toBytes((item._4.toString.substring(0,4))))
            myTable.put(p)
          }
        }
