...
object LinPack {
  def cal(SARDD:RDD[(String,String,String,String,String)], sc:SparkContext, config:Configuration)={
    ...
    val INRDD = SARDD.filter(item => (new String(item._3).equals("CPU")))
    val INRDD1 = INRDD.map(item => (item._1 , item._4.toDouble))
    //得到每个JOPS的一天总发生量
    val INZONGRDD = INRDD1.reduceByKey(_ + _).map(item => (item._1 , item._2/(25128000*3881))).cache()
      ...
      triple => {
        ...
        triple.foreach{
          item=>{
            ...
            p.add(Bytes.toBytes("ITEM"), Bytes.toBytes("LinPack_avg"),Bytes.toBytes(item._2.toString.substring(0,4)))       //写入平均值
            p.add(Bytes.toBytes("ITEM"), Bytes.toBytes("LinPack_max_1"),Bytes.toBytes((item._3.toString.substring(0,4))))
            p.add(Bytes.toBytes("ITEM"), Bytes.toBytes("LinPack_max_2"),Bytes.toBytes((item._4.toString.substring(0,4))))
            myTable.put(p)
          }
        }
